...
export default function EditEventPage() {
    ...
    fetch(`/api/events/${eventId}`)
      ...
      .then(ev => {
        setForm({
          name: ev.name || '',
          type: ev.type || 'draft',
          format: ev.format || '6v6',
          halfLength: ev.half_length || 20,
          slots: {
            rifle: ev.slots_rifle ?? 2,
            third: ev.slots_third ?? 1,
            heavy: ev.slots_heavy ?? 2,
            sniper: ev.slots_sniper ?? 1,
          },
  ...
  const debouncedSave = useCallback((f: FormState) => {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => saveToDb(f), 1200);
  }, [saveToDb]);
  ...
  const toggleMap = (m: string) => {
    if (!form) return;
    const next = form.maps.includes(m) ? form.maps.filter(x => x !== m) : [...form.maps, m];
      ...
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 32 }}>
          {['Basics', 'Details', 'Review'].map((label, i) => {
            ...
            const isDone = step > n;
            return (
              <div key={n} onClick={() => isDone ? goTo(n) : undefined} style={{ flex: 1, padding: '10px 0', textAlign: 'center', fontSize: 11, letterSpacing: '0.1em', fontFamily: 'var(--font-body)', cursor: isDone ? 'pointer' : 'default', color: isActive ? 'var(--khaki)' : isDone ? 'var(--green-light)' : 'var(--text-dim)', borderBottom: isActive ? '2px solid var(--khaki)' : '2px solid transparent', marginBottom: -1, transition: 'color 0.15s' }}>
                {n}. {label.toUpperCase()}
              </div>
            );
          })}
        ...
        {step === 1 && (
          <div>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 28, letterSpacing: '0.08em', color: 'var(--khaki)', marginBottom: 28 }}>BASICS</h2>

            <Field label="Event Name">
              <input style={inputStyle} type="text" placeholder="Friday Night Draft" value={form.name} onChange={e => updateForm({ name: e.target.value })} />
            </Field>

            <Field label="Type">
              <PillGroup options={[{ val: 'draft', label: 'Draft' }, { val: 'community', label: 'Community Event' }]} value={form.type} onChange={val => updateForm({ type: val })} />
            </Field>

            <Field label="Format">
              <PillGroup options={['6v6','8v8','12v12','16v16'].map(v => ({ val: v, label: v }))} value={form.format} onChange={val => setFormat(val)} />
            </Field>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              <Field label="Half Length">
                <PillGroup options={[{ val: '15', label: '15 min' }, { val: '20', label: '20 min' }]} value={String(form.halfLength)} onChange={val => updateForm({ halfLength: parseInt(val) })} />
              </Field>
        ...
        {step === 2 && (
          <div>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 28, letterSpacing: '0.08em', color: 'var(--khaki)', marginBottom: 28 }}>DETAILS</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <Field label="Check-in Opens">
                <input style={{ ...inputStyle, colorScheme: 'dark' }} type="datetime-local" value={form.checkinOpens} onChange={e => updateForm({ checkinOpens: e.target.value })} />
              </Field>
            </div>
        ...
        {step === 3 && (
          <div>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 28, letterSpacing: '0.08em', color: 'var(--khaki)', marginBottom: 28 }}>REVIEW</h2>
            ...
            {saving && <p style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 10, fontFamily: 'var(--font-body)', letterSpacing: '0.08em' }}>Saving...</p>}
...
const inputStyle: React.CSSProperties = {
  width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', padding: '10px 12px', fontSize: 14, fontFamily: 'var(--font-body)', outline: 'none', borderRadius: 0,
};

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <label style={{ fontSize: 10, letterSpacing: '0.14em', color: 'var(--text-dim)', marginBottom: 8, display: 'block', textTransform: 'uppercase', fontFamily: 'var(--font-body)' }}>{label}</label>
      {children}
    </div>
  );
}
...
function SlotBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ width: 22, height: 22, background: 'transparent', border: '1px solid var(--border-strong)', color: 'var(--khaki)', fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 2, fontFamily: 'monospace' }}>
      {children}
    </button>
  );
}
...
function BtnRow({ children }: { children: React.ReactNode }) {
  return <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 28 }}>{children}</div>;
}
