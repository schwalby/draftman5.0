...
const entries: Entry[] = [
  ...
  },
  {
    session: '23',
    date: 'June 1, 2026',
    heading: 'First live test. Everything breaks. Everything gets fixed.',
    body: 'Ran the first real draft with live players and collected a full list of failures. The biggest structural problem: when the admin clicked Lock In, only they were redirected to the draft board — captains had no way in and no idea it had started. Fixed by building a lobby system from scratch: a new draft_lobby table, three API routes (GET lobby state, POST open lobby, POST ready, POST start), and a /events/[id]/draft/lobby page where all captains meet, see their team assignment, and click Ready before the draft begins. When the admin locks in teams, the bot posts a @captains ping in #captains-chat with the lobby URL. The admin sees a live ready count and a Start Draft button that enables when everyone is confirmed — with a Force Start option for stragglers. Once started, all polling clients auto-redirect to the pick board simultaneously. The full event lifecycle is now formally tracked through status values: published → lobby → drafting → completed. Reset draft was overhauled to match — it now clears picks, clears lobby ready states, sets status back to lobby, and reposts the Discord notification in one action, so everyone on the pick board auto-redirects back to the lobby without the admin having to do anything else. A race condition was caught where two admins (everyone had been made admin for testing) confirmed different players at the same time and both landed on the same team — the picks API now returns a friendly error and auto-refreshes stale team data on the client. The teams API was guarded to block re-saves when a draft is in progress, which was the root cause of a foreign key constraint error players saw mid-draft. Terminology was swept to match community language: the process of assigning players to teams is Picking Teams, not drafting — Pick Board, Finish Picking, Lock In & Open Lobby, Rejoin Picks, Team Summary, Picking in Progress all updated across the UI. Backend names unchanged. The events page in-progress badges were made accurate per lifecycle status: Lobby Open links to the lobby, Picking Teams links to the pick board, Draft in Progress links to the tournament. The dashboard was fixed to recognise drafting and lobby as in-progress states — both were previously falling through to the unpublished bucket. Finally, the Steam verify routes had been redirecting all error paths to localhost — req.url on Railway is the internal address, not the public one. Every redirect in the callback and start routes now uses the public host derived from x-forwarded-host headers.',
    tags: ['Draft lobby', 'Ready-check system', 'Bot lobby notification', 'Lifecycle status', 'Reset overhauled', 'Race condition guard', 'FK constraint fix', 'Terminology sweep', 'Accurate badges', 'Steam verify localhost fix', 'Dashboard status fix'],
  },
  {
    session: '24',
    date: 'June 12, 2026',
    heading: 'A clean slate, a full audit, and a hardening pass.',
    body: 'Started from a clean slate — every event, signup, team, pick, and tournament from the first live test was purged by hand so real usage could begin on fresh data. Then ran a full architectural review of the entire platform — bot and web app, every route, every state transition — and turned the findings into a permanent wiki: a system map, the review itself, an incremental remediation plan, and a two-year growth analysis, so the work survives across sessions. From there, a hardening pass, lowest-risk first, database layer before code. Stood up a real migrations system — the schema had only ever lived in the production database, so it got dumped as migration zero and every change since is a numbered, version-controlled file. Tightened up the data layer: writes are locked to the server side, a guard makes sure a player can\'t hold two active signups for one event, and the audit trail now reliably records every meaningful action. Then the highest-value code fix: the tournament bracket. Confirming a match dropped the winner into the first empty slot of the next match, so a double-click could put one team into both halves of a semifinal, editing a result after it advanced appended the new winner without clearing the old one, and rejecting an advanced result left the team standing in the next round forever. Advancement is deterministic now — a match\'s number fixes exactly which slot it feeds — with a guard so a match can only be confirmed once, and rejecting walks the advancement back. A match the score bot reports with no declared winner now derives one from the score instead of completing as a null, and captains can only confirm matches their own team actually played in. Closed out with two smaller ones: the user-edit endpoint now accepts only an explicit set of fields instead of whatever it is handed, and the check-in announcement finally counts who is actually checked in instead of everyone who signed up.',
    tags: ['Clean-slate purge', 'Architecture review', 'Remediation wiki', 'Migrations system', 'Schema baseline', 'Data layer hardened', 'Audit trail reliable', 'Signup uniqueness', 'Bracket advancement rewrite', 'Confirm-once guard', 'Captain scoping', 'PATCH allowlist', 'Check-in count fix'],
  },
];
